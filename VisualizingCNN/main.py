...
def load_images(img_path):
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),  # 1、把 PIL图片/numpy数组→PyTorch张量Tensor。2、把 PIL图片/numpy数组→PyTorch张量Tensor。3、把像素值从0–255归一化到0–1
        transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]) # 归一化 ： (x - mean)/std
        ])
